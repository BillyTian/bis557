...
## Question 2
A new function fitting the OLS model using gradient descent that calculates the loss based on the out-of-sample accuracy is made. Please check the "new_gradient_descent" funciton in R folder. (It takes 5-10 minutes to run this on my laptop because I use the mean squared residual computed from the cross validation as the measurement of out-of-sample accuracy. So, if necessary, please be careful to run. Sorry for any inconvenience brought to the grader.)$\\$
The corresponding test code called "test-new-gradient-descent" is in the testthat folder.$\\$

Here, I show the comparison to the OLS model ("gradient_descent" and "lm"):
```{r}
data(iris)
new.gd <- new_gradient_descent(Sepal.Length ~ ., iris)$coefficients
